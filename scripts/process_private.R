...
masked_maakID_sf <- st_read(dsn = private_sf_path, layer = paste0("masked_polygon_maakID_", config$municipID), drivers = "ESRI Shapefile")
...
colnames(private_n_dt)[which(colnames(private_n_dt) == "VALUE")] <- "segID"
...
colnames(all_n)[which(colnames(all_n) == "VALUE")] <- "segID"
...
# Public N
all_private_n_dt <- left_join(all_n, private_n_dt, by = "segID")
all_private_n_dt[is.na(N.y), N.y:=0]
colnames(all_private_n_dt) <- c("segID", "all_N", "private_N")
...
all_private_n_dt[segID %in% under_10$segID, private_N := 0]
all_private_n_dt[segID %in% over_10$segID, private_N := all_N]
